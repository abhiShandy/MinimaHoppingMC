...
class CarrierHoppingMC:
	...
	def __init__(self, params):
		...
		nParticles = np.round(np.prod(L)*volFracNP/(4./3*np.pi*(radiusNP)**3)).astype(int) #total number of nano-particles
	...
	def run(self, iRun=0):
		...
		trajectory = [ (iElectron+nElectronOffset, t) + tuple(iPos*self.h) for iElectron,iPos in enumerate(iPosElectron.T) ]
		...
		while len(trajectory) < self.maxHops:
			...
			trajectory.append((iHop+nElectronOffset, t) + tuple(iPosNew*self.h))
			if iPosNew[2] > izMax:
				izMax = iPosNew[2]
				print "Run", iRun, "reached", izMax*self.h, "nm at t = {:.2e}s".format(t)
		...
		print 'End MC run', iRun, 'with trajectory length:', len(trajectory), 'events, reached', izMax*self.h, "nm at t = {:.2e}s".format(t)
